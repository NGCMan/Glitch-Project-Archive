...
function gameLoop() {
  ...
  document.getElementById(
    "AntimatterAmount"
  ).innerText = `You have ${shortenMoney(player.antiMatter)} antimatter.`;

  document.getElementById(
    "AntimatterMulti"
  ).innerText = `Your antimatter production multiplier is ${shorten(
    getAntiGrindMulti()
  )}x`;

  document.getElementById(
    "metaShiftBtn"
  ).innerText = `Meta Shift. Cost: ${shorten(
    player.metaShiftCost
  )} antimatter.`;

  for (let i = 1; i < 11; i++) {
    document.getElementById(`dim${i}stats`).innerText = `You have ${shorten(
      player.ndAmounts[i].floor()
    )} ${getDimTitles(i)} dimensions. They have a ${shorten(
      player.ndMults[i]
    )}x multiplier. ${player.ndBoughts[i]} bought. Cost: ${shorten(
      player.ndCosts[i]
    )} antimatter.`;
