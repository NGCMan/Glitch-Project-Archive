...
function getDefaultSave() {
	return {
			...
			},
      {
		...
		treeUpgrades: [],
		autos: [false,false,false,false,false,false,false,false,false],
...
function update() {
	...
	get("thonkEffect").innerHTML = format(player.thonks.add(1).log(23).root(1.25),true);
	let unlocked7 = false;
	for(let i = 1; i <= 9; i++) {
	...
	if(!player.thonks.add(1).log(23).root(1.25).gt(1)) {
    get("manifolds").innerHTML = format(player.manifolds,true);
  } else {
    get("manifolds").innerHTML = format(player.manifolds,true) + " + " + format(player.thonks.add(1).log(23).root(1.25), true);
  }
...
function mults() {
	...
	player.manifoldMult = new Decimal(1.2).pow(player.manifolds.add(player.thonks.add(1).log(23).root(1.25).floor()));
	player.manifoldMult2 = new Decimal(1.05).pow(player.manifolds.add(player.thonks.add(1).log(23).root(1.25).floor()).cbrt());
	if(player.treeUpgrades.includes("st22")) {
	...
	if(free) {
		...
		if(player.manifoldCost.gte(16)) {
			if(player.treeUpgrades.includes("st21")) {
							if(player.treeUpgrades.includes("st82")) {
					  		if(player.gravityUpgrades.includes("sg32")) {
			          	player.manifoldCostMult = player.manifoldCostMult.times(player.manifoldCostScale.div(1.89));
		          	} else {
...
function abstract(grav = false) {
  ...
  player.thinkers[9] = getDefaultSave().thinkers[9];
